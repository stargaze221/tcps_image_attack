...
class ImageAttackTraniner:
    ...
    def __init__(self, setting_dict=SETTING):
        ...
        ### Yolo Model ###
        self.yolo_model = YoloWrapper('yolov5m.pt')
...
class ImageAttacker():
    ...
    def generate_attack(self, obs, tgt_box):
        ...
        #print('image_torch', image_torch.size(), image_torch.min(), image_torch.max())
        ...
        #print('action_torch', action_torch.size(), action_torch.min(), action_torch.max())
        ...
        pred = YOLO_MODEL.get_predictions(X_attacked)
        print('X_attacked', pred)
        pred = YOLO_MODEL.get_predictions(X)
        print('X', pred)

        attacked_obs = X_attacked.detach().squeeze().permute(1, 2, 0).cpu().numpy()
        return attacked_obs
